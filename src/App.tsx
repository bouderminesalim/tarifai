...
function Footer() { return <footer className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8"><div className="rounded-[2rem] bg-slate-950 p-8 text-white dark:bg-white/10"><div className="grid gap-8 md:grid-cols-4"><div><h3 className="text-2xl font-black">TarifAI</h3><p className="mt-3 text-sm text-white/60">Architecture cible: React/Next, NestJS, PostgreSQL, Redis, AWS/Azure, REST+GraphQL, Swagger, Docker, Kubernetes, CI/CD et Grafana.</p></div>{[['Sécurité',[<ShieldCheck key="s"/>, 'WCAG, RLS, audit logs']], ['Paiements',[<CreditCard key="c"/>, 'Stripe, PayPal, Visa, Mastercard, CIB, Edahabia, SATIM']], ['Global',[<Globe2 key="g"/>, 'FR, EN, AR']], ['Alertes',[<Bell key="b"/>, 'Email, notifications, SLA']]].map(([title, body]) => <div key={title as string}><h4 className="font-black">{title}</h4><p className="mt-3 flex items-center gap-2 text-sm text-white/60">{body as React.ReactNode}</p></div>)}</div></div></footer>; }
