...
export function signInWithGoogle(appName = 'TarifAI') {
  ...
  const handler = async (event: MessageEvent) => {
    if ((event.data as { type?: string })?.type === 'google-auth-denied') {
      window.removeEventListener('message', handler);
      return;
    }
    if ((event.data as { type?: string })?.type !== 'google-auth-success') return;
    window.removeEventListener('message', handler);
    const data = event.data as { access_token?: string; refresh_token?: string; id_token?: string };
    if (data.access_token && data.refresh_token) {
      ...
      const { error } = await supabase.auth.signInWithIdToken({ provider: 'google', token: data.id_token });
