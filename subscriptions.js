...
export default async function handler(req, res) {
  ...
  try {
    if (req.method === 'GET') {
      const { data, error } = await supabase.from('subscriptions').select('*').order('monthly_price', { ascending: true });
      if (error) throw error;
      return res.status(200).json(data);
    }
    if (req.method === 'POST') {
      const payload = req.body;
      if (!payload.plan_name) return res.status(400).json({ error: 'Nom du plan requis' });
